...
@Component
public class ContractTimestampDAO extends ParentDAO<TimestampModel> {
    @Autowired
    public ContractTimestampDAO(JdbcTemplate jdbcTemplate) {
        super(jdbcTemplate);
    }

    @Override
    protected String getTableName() {
        return "contract_timestamps";
    };
    ...
    protected void getValidation(TimestampModel model) {
        if (model == null)
            throw new IllegalArgumentException("Contract timestamp should not be null");
        if (model.getParentId() == null) {
            String message = "Contract currency id should not be null: ";
            message += "model=" + model;
    ...
    protected void updateValidation(TimestampModel model) {
        getValidation(model);
        if (model.getId() == null)
            throw new IllegalArgumentException("Contract timestamp id should not be null: " + model);
    }
