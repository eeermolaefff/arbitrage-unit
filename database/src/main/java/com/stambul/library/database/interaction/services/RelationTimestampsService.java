...
import com.stambul.library.database.interaction.dao.RelationTimestampDAO;
...
@Component
public class RelationTimestampsService extends ParentService<TimestampModel, TimestampDTO> {

    @Autowired
    public RelationTimestampsService(
            RelationTimestampDAO relationTimestampDAO,
            @Value("${database.batch.size}") int batchSize
    ) {
        super(relationTimestampDAO, batchSize);
    }
    ...
    public List<TimestampDTO> getByCurrencyIds(Iterable<Integer> ids) {
        ...
        return toDTOsList(((RelationTimestampDAO) dao).getByCurrencyId(ids));
    }
    ...
    protected class ListHandler implements Handler<Iterable<Integer>> {
        ...
        public void handle(Iterable<Integer> ids) {
            this.list.addAll(RelationTimestampsService.this.getByCurrencyIds(ids));
        }
